...
function citizenPayload() {
  ...
  return {
    ...
    messages: state.messages.filter((item) => item.citizen_id === profile.id), slots: state.slots, privacyRequests: [],
  };
}
...
function adminPayload() {
  ...
  return { ok:true, me:state.adminUsers[0], products:state.products, lots:state.lots, requests:state.requests, slots:state.slots,
    ...
    chats, adminMessages:state.messages, auditEvents:[{id:"audit-demo",actor:"admin-gabriela",action:"login",entity:"admin_user",entity_id:"admin-gabriela",details:"Acesso demonstrativo",created_at:timestamp()}], adminUsers:state.adminUsers, privacyRequests:[],
...
async function post(body: Record<string, unknown>) {
  ...
  if (action === "subscribePush") return message("Alertas ativados neste aparelho.");
  if (action === "unsubscribePush") return message("Alertas desativados neste aparelho.");
  if (action === "requestStatus") { if(body.status==="rejected"&&!String(body.reason??"").trim())return response({ok:false,message:"Informe o motivo da não aprovação."},400); updateStatus(body.requestId,String(body.status)); const item=state.requests.find((entry)=>entry.id===body.requestId);if(item&&body.reason)Object.assign(item,{decision_reason:String(body.reason)}); return message("Situação da solicitação atualizada com rastreabilidade."); }
  ...
  if (action === "validateCitizen") { const item=state.citizens.find((entry)=>entry.id===body.citizenId); if(item)item.validation_status="validated"; return message("Documentos validados. A entrega será liberada após a primeira retirada presencial."); }
  ...
  if (action === "privacyRequest") return message("Solicitação de privacidade registrada para análise da prefeitura.");
  if (action === "privacyStatus") return message("Solicitação de privacidade atualizada.");
...
export function installDemoApi() {
  ...
  window.fetch = async (input, init) => {
    ...
    if (action === "pushConfig") return state.session === "citizen" ? response({ok:true,configured:false,publicKey:null,activeDevices:0}) : response({ok:false},401);
