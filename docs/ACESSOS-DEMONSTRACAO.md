# Acessos de demonstração

Todos os dados abaixo são fictícios e exclusivos da demonstração.

## Cidadã validada

- CPF: `123.456.789-09`
- Senha: `Cidadao@2026`

## Nova ativação

- CPF: `111.222.333-44`
- Nascimento: `18/06/1972`
- Código: `SAUDE-2026`
- A senha é criada durante o fluxo de ativação.

## Farmácia

- Usuário: `admin@altair.sp.gov.br`
- Senha: `Admin@2026`

## Cenário carregado

- 24 medicamentos com lotes, saldos e validades variados.
- 15 cidadãos fictícios em diferentes situações cadastrais.
- Solicitações para retirada e entrega em múltiplas etapas.
- Demandas e agendas para Cardiologia, Ortopedia, Oftalmologia e Dermatologia.
